...
| 3 | Geo-Targeted Subscriber Identification | GREEN | **DB-aggregate** `SubscriberCellStatsService.java:303 O(k)` `SubscriberMatcherTest.java:1` (5 tests) + `KpiOptimizedSubscriberBenchmarkTest.java:1` H2 100k~100M via `turant_agg` (20k 449ms/50k 1444ms/100k 1987ms warmMed <60s) + prod `.benchmark-subscriber-matching.md:16` 50k 40ms (97M rows) — **VLR hash** `VlrProbeService.java:44 O(N+K)` `VlrProbeServiceTest.java:1` (7 tests) + `KpiVlrHashSemiJoinBenchmarkTest.java:1` file-based gz 100k*100=10M scaled (20k 2235ms/50k 2986ms/100k 4519ms warmMed <60s, full 100M est. 11s/15s/22s, scaled 10x for CI heap/time, traceable `kpi-vlr-benchmark-*.json`) — both PASS, not conflated |
